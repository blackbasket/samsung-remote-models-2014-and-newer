...
messages.on(
  (data) => data.startsWith('5::/com.samsung.companion:'),
  (message) => {
    const { socket, identity, data } = message
    ...
    try {
      ...
      const decrypted = JSON.parse(
        Encryption.decryptData(identity.aesKey, event.args)
      )
...
const openSocket = (config, identity, eventEmitter) => {
  return fetch(
    ...
    .then((mask) => {
      ...
      socket.on('error', (err) => {
        console.error('An error has occurred with the socket connection', err) // eslint-disable-line
      })

      return socket
    })
    .catch((err) => {
      console.error('Error while creating socket', err) // eslint-disable-line
    })
}
...
class SamsungTvConnection {
  ...
  connect() {
    return openSocket(this.config, this.identity, this.eventEmitter).then(
      (socket) => {
